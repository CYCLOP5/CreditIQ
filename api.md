...
| top_reasons | six plain english sentences explaining the primary drivers of the score (5 from MiniMax-01 via OpenRouter API/shap + 1 actionable 'path to prime' step), generated by MiniMax-01 via OpenRouter API from shap values |
...
the saga worker calls [`src/llm/translator.py`](../src/llm/translator.py) to produce the five top_reasons bullets. MiniMax-01 via OpenRouter API requires the gguf model file at `data/models/MiniMax-01 via OpenRouter API-mini-128k-instruct-cloud endpoint.gguf`.

### when MiniMax-01 via OpenRouter API gguf is present
...
### when MiniMax-01 via OpenRouter API gguf is absent
...
| `post` | `/score/{task_id}/chat` | `ChatRequest` -> `text/event-stream` | rag-based MiniMax-01 via OpenRouter API credit analyst q&a |
