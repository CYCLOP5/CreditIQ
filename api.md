...
| top_reasons | six plain english sentences explaining the primary drivers of the score (5 from gemma via openrouter/shap + 1 actionable 'path to prime' step), generated from shap values |
...
the saga worker calls [`src/llm/translator.py`](../src/llm/translator.py) to produce the five top_reasons bullets. it uses gemma via the openrouter cloud api.

### when openrouter is configured

the worker sends requests to openrouter (cloud-hosted gemma model) at scoring time. the prompt passes the top 5 shap feature names, values, and direction labels. the model returns exactly 5 bullet points in plain language plus a 6th 'path to prime' actionable step. parse_llm_output in [`src/llm/prompts.py`](../src/llm/prompts.py) strips markdown formatting and pads or truncates to exactly 6 items.

### when openrouter is unavailable

if the api key is missing, the model provider is unavailable, or a network error occurs, `shaptranslator.translate` falls back to generating top_reasons directly from feature names and direction labels without any llm call. the output is less fluent but structurally correct. no exception propagates. the score, risk band, and all other fields are unaffected.
...
| `post` | `/score/{task_id}/chat` | `ChatRequest` -> `text/event-stream` | rag-based gemma via openrouter cloud credit analyst q&a |
